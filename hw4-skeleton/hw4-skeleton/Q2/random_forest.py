...
class RandomForest(object):
    ...
    def __init__(self, num_trees):
        ...
        self.decision_trees = [DecisionTree(max_depth=1) for i in range(num_trees)]
    ...
    def _bootstrapping(self, XX, n):
        ...
        #############################################
        rows = np.random.randint(low=0, high=len(XX), size=n)
        d = len(XX[0])-1
        for r in rows:   
            samples.append(XX[r][:d])
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                #############################################
                special += 1
                classification = self.decision_trees[0].classify(record)
        ...
        print("Special: {}, Non: {}".format(special, non_special))       
        return y
