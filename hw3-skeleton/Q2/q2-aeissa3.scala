...
df.filter(df("PULocationID") =!= df("DOLocationID") && df("trip_distance") > 2.0).createOrReplaceTempView("dff")
...
dff.groupBy(dff("DOLocationID")).count().orderBy(desc("count"), asc("DOLocationID")).withColumnRenamed("count", "number_of_dropoffs").show(5)
...
dff.groupBy(dff("PULocationID")).count().orderBy(desc("count"), asc("PULocationID")).withColumnRenamed("count", "number_of_pickups").show(5)
...
dff.groupBy(dff("PULocationID"), dff("DOLocationID")).count().orderBy(desc("count"), asc("PULocationID")).withColumnRenamed("count", "count of pickup-dropoff").show(5)
...
val dff2 = dff.withColumn("date", date_format(to_date($"dropoff_datetime"), "MM-dd-YYYY")).withColumn("year", date_format(to_date($"dropoff_datetime"), "YYYY")).withColumn("month", date_format(to_date($"dropoff_datetime"), "MM")).withColumn("day", date_format(to_date($"dropoff_datetime"), "dd"))

dff2.filter(((dff2("month") === "01") && (dff2("day") < "06" ))&& dff2("year") === "2019").withColumn("date", concat_ws("-", $"year", $"month", $"day")).groupBy($"date").count().orderBy($"date").show()
...
val pick = dff.groupBy(dff("PULocationID")).count().withColumnRenamed("count", "pickups")
val drop = dff.groupBy(dff("DOLocationID")).count().withColumnRenamed("count", "dropoffs")
val both = pick.join(drop, col("PULocationID")=== col("DOLocationID"))
val newboth = both.withColumn("total_activity", both.columns.map(c => col(c)).reduce((c2, c4) => c2 + c4)).withColumnRenamed("PULocationID", "locationID").drop("pickups").drop("dropoffs").drop("DOLocationID").orderBy(desc("total_activity"), asc("locationID")).show(3)
